...
public class MonthView extends JPanel{
	...
	public MonthView(StateMachine SM, User user){
		...
		while (true) {
			...
			tempJP.add(new DayButton(Integer.toString(j),checkDate,1,SM), BorderLayout.PAGE_START);
				...
				if(checkDate.equals(checkEventDayStart)||checkDate.equals(checkEventDayEnd)||checkIfInProgress(checkDate, checkEventDayStart, checkEventDayEnd)){
							if(filteredEventArray[i].getFullDay()==1){
								if(tempFullDayEvents==1){
									...
									tempJP.add(new DayButton("...", checkDate, 3,SM), BorderLayout.PAGE_END);
									tempFullDayEvents=-1;
								}
							}else{
								...
								if(gbc.gridx>=2){
									normalEventsPanel.add(new DayButton("...", checkDate, 2,SM), gbc);
								}else{
