...
public class DayPanel extends JPanel {
	...
	public DayPanel(Event[] eventArray, String date, int h, boolean inweekview, StateMachine SM, WindowPanel wp,
			User user) {
		...
		for (int i = 0; i < fullDay.length; i++) {
			...
			fullDayEvents.add(new FullDayEventWeek(fullDay[i].getName(), fullDay[i], SM, wp, user));
		}
		...
		for (int i = 0; i < normal.length; i++) {
			...
			if (checkIfInProgress(normal[i])) {
				...
				NormalEventWeek tempEventButton = new NormalEventWeek(normal[i].getName(), normal[i], SM, wp, user);
	...
	private boolean checkIfInProgress(Event event) {
		...
		if ((Integer.parseInt(tempStart) < Integer.parseInt(date))
				&& (Integer.parseInt(tempEnd) > Integer.parseInt(date))) {
			return true;
		} else {
			return false;
	...
	private int getLength(Event event) {
		...
		if ((Integer.parseInt(date) == Integer.parseInt(tempStartDate))
				&& (Integer.parseInt(date) == Integer.parseInt(tempEndDate))) {
			int hour = Integer.parseInt(tempEndTime.substring(11, 13))
					- Integer.parseInt(tempStartTime.substring(11, 13));
			int halfHour = Integer.parseInt(tempEndTime.substring(14, 16))
					- Integer.parseInt(tempStartTime.substring(14, 16));
			int startTime = 0;
			// hour
			startTime += (24.7 * hour);
			// half hour
